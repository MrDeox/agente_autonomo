...
class OptimizedAgentInitializer:
    ...
    async def _init_hephaestus_agent(self) -> Dict[str, Any]:
...
async def rate_limiting_middleware(request: Request, call_next):
    ...
    try:
        ...
        if auth_manager and not auth_manager.check_rate_limit(client_ip):
            return JSONResponse(
                status_code=429,
...
def get_auth_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        # Check rate limiting
        if auth_manager and not auth_manager.check_rate_limit(token_payload.user_id):
            raise HTTPException(
...
async def get_status():
    ...
    return {
        ...
        "performance_metrics": {
            "rate_limit_storage_size": 0,  # Rate limiting storage not initialized
